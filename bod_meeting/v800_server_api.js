...
function getDashboardUrl() {
  ...
  try {
    ...
    var settingsSheet = ss.getSheetByName(CONFIG.SHEET_CONFIG);
    if (settingsSheet) {
...
function getProcessStatus() {
  ...
  // ===== XÁC ĐỊNH GIAI ĐOẠN HIỆN TẠI THEO THỜI GIAN =====
  // T3–T6 (day 2–5) → Bước 1: Đăng ký
  // T7 (day 6) → Bước 2: Duyệt
  // CN trước 19:00 (day 0, hour < 19) → Bước 2: Duyệt (tiếp)
  // CN từ 19:00 (day 0, hour >= 19) → Bước 3: Lịch trình
  // T2 (day 1) → Bước 4: Họp BOD
  var timeStep;
  if (day >= 2 && day <= 5) timeStep = 1;
  else if (day === 6) timeStep = 2;
  else if (day === 0 && hour < 19) timeStep = 2;
  else if (day === 0 && hour >= 19) timeStep = 3;
  else timeStep = 4; // day === 1 (Thứ 2)
  ...
  if (scheduleSheet) {
    ...
    if (schedDate.indexOf(searchDate) >= 0) {
      ...
      scheduleApproved = banner.indexOf("ĐÃ DUYỆT") >= 0;
    }
  }

  // ===== GÁN TRẠNG THÁI CÁC BƯỚC =====
  // Bước trước timeStep → done, bước = timeStep → active, bước sau → pending
  // Overlay dữ liệu thực tế lên trạng thái thời gian
  result.currentStep = timeStep;
  ...
  if (timeStep > 2) {
    ...
    // Nếu tất cả đã duyệt xong → done (hoàn thành sớm)
    if (total > 0 && pending === 0 && approved > 0) {
      result.steps[1].status = "done";
    } else {
  ...
  if (timeStep > 3) {
    ...
    result.steps[2].count = scheduleCreated ? 1 : 0;
  } else if (timeStep === 3) {
    if (scheduleApproved) {
      result.steps[2].status = "done";
  ...
  if (timeStep === 4) {
    ...
    result.steps[3].status = "pending";
  }

  // Thông tin bổ sung cho frontend toast
  result.total = total;
  result.approved = approved;
  result.pending = pending;
  result.scheduleCreated = scheduleCreated;
  result.scheduleApproved = scheduleApproved;
...
function getDeptRegistrationStatus(searchDate) {
  ...
  // FALLBACK: Đọc email/tên đại diện từ "Nhân sự bắt buộc" khi "Dim bộ phận" thiếu
  try {
    var hrSheet = ss.getSheetByName(CONFIG.SHEET_HR);
    if (hrSheet && hrSheet.getLastRow() > 1) {
      var hrData = hrSheet.getDataRange().getValues();
      var colDept = CONFIG.HR_COL_DEPT - 1;   // Cột D (0-indexed = 3)
      var colEmail = CONFIG.HR_COL_EMAIL - 1;  // Cột F (0-indexed = 5)
      var colName = CONFIG.HR_COL_NAME - 1;    // Cột C (0-indexed = 2)
      for (var i = 1; i < hrData.length; i++) {
        var hrBP = (hrData[i][colDept] || "").toString().trim().toUpperCase();
        if (!hrBP) continue;
        // Match HR dept name → required dept key
        for (var j = 0; j < requiredDepts.length; j++) {
          var rk = requiredDepts[j].toUpperCase();
          if (hrBP === rk || hrBP.indexOf(rk) >= 0 || (rk.indexOf(hrBP) >= 0 && hrBP.length >= 2)) {
            var hrEmail = (hrData[i][colEmail] || "").toString().trim();
            var hrName = (hrData[i][colName] || "").toString().trim();
            // Chỉ bổ sung khi Dim bộ phận chưa có
            if (!deptEmails[rk] && hrEmail) deptEmails[rk] = hrEmail;
            if (!deptContacts[rk] && hrName) deptContacts[rk] = hrName;
            break;
          }
        }
      }
    ...
    Logger.log("[getDeptRegistrationStatus] HR fallback error: " + hrErr.message);
  }

  // Tạo danh sách key bộ phận bắt buộc để matching linh hoạt
  var requiredKeys = requiredDepts.map(function(d) { return d.toUpperCase(); });

  // Hàm match tên bộ phận linh hoạt: exact → contains → contained-by
  function matchDeptName_(formBP) {
    if (!formBP) return null;
    var bp = formBP.toUpperCase();
    // 1. Exact match
    if (requiredKeys.indexOf(bp) >= 0) return bp;
    // 2. Form chứa tên Dim (vd: form="TRUNG TÂM MSA", dim="MSA")
    for (var j = 0; j < requiredKeys.length; j++) {
      if (bp.indexOf(requiredKeys[j]) >= 0) return requiredKeys[j];
    }
    // 3. Dim chứa tên form (vd: form="MSA", dim="TRUNG TÂM MSA")
    for (var j = 0; j < requiredKeys.length; j++) {
      if (requiredKeys[j].indexOf(bp) >= 0 && bp.length >= 2) return requiredKeys[j];
    }
    return bp; // fallback: giữ nguyên (sẽ không match → chưa đăng ký)
  }
  ...
  if (sheet) {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      var bpRaw = (data[i][cols.boPhan] || "").toString().trim();
      var bp = matchDeptName_(bpRaw);
      if (!deptStats[bp])
  ...
  try {
    ...
    // Window = 7 ngày trước searchDate → searchDate (nhắc nhở gửi TRƯỚC ngày họp)
    var weekMonday = new Date(sDate);
    weekMonday.setDate(sDate.getDate() - 7);
    weekMonday.setHours(0, 0, 0, 0);
    var weekSunday = new Date(sDate);
...
function sendBulkReminderWeb(searchDate, displayDate) {
  ...
  for (var i = 0; i < depts.length; i++) {
    if (depts[i].status === "missing" && depts[i].email) {
      var r = sendDeptReminderWeb(depts[i].name, depts[i].email, displayDate, depts[i].contact, depts[i].cc, (depts[i].reminderCount || 0) + 1);
      if (r.success) {
        sent++;
