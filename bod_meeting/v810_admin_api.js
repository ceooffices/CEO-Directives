...
function saveEmailTemplate(key, jsonTemplate) {
  try {
    ...
    var sheet = getOrCreateSheet_(CONFIG.SHEET_CONFIG, ["Key", "Value"]);
...
/**
 * Version đơn giản cho Dashboard — đọc từ sheet "Settings"
 * Sheet Settings cần có:
 *   cfg_bod_hosting_name  | Lê Tuấn
 *   cfg_bod_hosting_time  | 03/2026
 * @returns {object} {name, period}
 */
function getBodHostingSimple() {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    if (settingsSheet) {
      ...
      for (var s = 0; s < sData.length; s++) {
        var k = (sData[s][0] || "").toString().trim().toLowerCase();
        var v = (sData[s][1] || "").toString().trim();
        if (k === "cfg_bod_hosting_name" && v) hostName = v;
        if (k === "cfg_bod_hosting_time" && v) hostPeriod = v;
      }
      if (hostName) return {name: hostName, period: hostPeriod};
    }
    return {name: "Chưa thiết lập", period: ""};
  } catch (e) {
    Logger.log("getBodHostingSimple error: " + e.message);
    return {name: "Chưa thiết lập", period: ""};
  }
}
...
// logEmailSend đã chuyển sang v852_email_router.js
