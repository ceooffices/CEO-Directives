/**
 * bod-import.js
 * Parse BOD review.md → JSON cấu trúc + import vào Supabase
 *
 ...
 *   - Supabase directives records
 *
 * Usage:
 *   node bod-import.js --file ban_chep_loi/BOD_16032026.review.md              # Parse + Supabase import
 *   node bod-import.js --file ban_chep_loi/BOD_16032026.review.md --json-only  # Parse → JSON only (no DB)
...
// NOTE: notion-client removed — Supabase is the single source of truth (migrated 2026-03-20)
...
const IMPORT_SUPABASE = !JSON_ONLY; // Default: always import to Supabase
...
// NOTE: Notion writer removed — Supabase is the single source of truth (migrated 2026-03-20)
// Old createClarificationRecord() function deleted — use Supabase insert in step 5
...
async function run() {
  ...
  if (fileIdx === -1 || !process.argv[fileIdx + 1]) {
    console.error('Usage: node bod-import.js --file <path.review.md> [--json-only] [--dry-run]');
    process.exit(1);
  }

  const filePath = path.resolve(process.argv[fileIdx + 1]);

  const modeLabel = DRY_RUN ? '▫️ DRY-RUN' : IMPORT_SUPABASE ? '► LIVE → Supabase' : '► JSON only';
  console.log('==========================================');
  console.log('[BOD-IMPORT] BOD Review.md → JSON + Supabase');
  console.log(`[BOD-IMPORT] Mode: ${modeLabel}`);
  console.log(`[BOD-IMPORT] File: ${filePath}`);
  console.log('==========================================');

  // 1. Đọc file
  console.log('\n[1/4] Loading review file...');
  ...
  console.log('\n[4/4] Writing output + Supabase import...');
  if (!DRY_RUN) {
  ...
  // NOTE: Notion import removed — Supabase is the single source of truth (migrated 2026-03-20)

  // Log execution
  try {
    await logExecution({
      workflow: 'BOD Import',
      step: 'Review.md → JSON + Supabase',
      status: (supaErrors > 0) ? '⚠️ Warning' : '✅ Success',
      details: `File: ${path.basename(filePath)} | Directives: ${result.total_directives} | Supabase: ${supaCreated}/${supaErrors}`,
      dryRun: DRY_RUN,
    });
  } catch (err) {
